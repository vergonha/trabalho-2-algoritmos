...
void gerar_usuarios_indice_exaustivo_area_extensao() {
  ...
  FILE *indice =
      fopen("./indices/indice_usuarios_exaustivo_area_de_extensao.bin", "wb");
  if (indice == NULL) {
  ...
  FILE *area_de_extensao =
      fopen("./areas_extensao/usuarios_area_extensao.bin", "rb");
  if (area_de_extensao == NULL) {
    perror("Failed to open index file for writing");
    free(usuario);
    return;
  }
  fseek(area_de_extensao, 0, SEEK_END);
  int quantidade_de_usuarios_area_de_extensao =
      ftell(area_de_extensao) / sizeof(Usuario);
  ...
  IndiceUsuario *indices =
      malloc((quantidade_de_usuarios_area_de_extensao) * sizeof(IndiceUsuario));
  if (indices == NULL) {
...
void gerar_produtos_indice_exaustivo_area_extensao() {
  ...
  FILE *indice =
      fopen("./indices/indice_produtos_exaustivo_area_de_extensao.bin", "wb");
  if (indice == NULL) {
    perror("Failed to open index file for writing");
    free(produto);
    return;
  ...
  FILE *area_de_extensao =
      fopen("./areas_extensao/produtos_area_extensao.bin", "rb");
  fseek(area_de_extensao, 0, SEEK_END);
  int quantidade_de_produtos_area_de_extensao =
      ftell(area_de_extensao) / sizeof(Produto);
  rewind(area_de_extensao);
  ...
  IndiceProduto *indices =
      malloc((quantidade_de_produtos_area_de_extensao) * sizeof(IndiceProduto));
  if (indices == NULL) {
    perror("Failed to allocate memory for indices");
    free(produto);
    fclose(indice);
...
IndiceUsuario *
buscar_usuarios_excluidos(int *quantidade_de_usuarios_excluidos) {
  FILE *indice_exaustivo =
      fopen("./indices/indice_usuarios_exaustivo.bin", "rb");
  ...
  while (fread(&buffer, sizeof(IndiceUsuario), 1, indice_exaustivo)) {
    if (buffer.deletado == 1) {
      ...
      IndiceUsuario *temp =
          realloc(usuarios_excluidos, (counter + 1) * sizeof(IndiceUsuario));
      if (temp == NULL) {
...
IndiceProduto *
buscar_produtos_excluidos(int *quantidade_de_produtos_excluidos) {
  FILE *indice_exaustivo =
      fopen("./indices/indice_produtos_exaustivo.bin", "rb");
  if (indice_exaustivo == NULL) {
    perror("Failed to open index file for reading");
    return NULL;
  }
  ...
  while (fread(&buffer, sizeof(IndiceProduto), 1, indice_exaustivo)) {
    if (buffer.deletado == 1) {
      ...
      IndiceProduto *temp =
          realloc(produtos_excluidos, (counter + 1) * sizeof(IndiceProduto));
      if (temp == NULL) {
...
void gerar_produtos_indice_exaustivo() {
  ...
  IndiceProduto *excluidos =
      buscar_produtos_excluidos(&quantidade_de_produtos_excluidos);
...
void gerar_usuarios_indice_exaustivo() {
  ...
  IndiceUsuario *excluidos =
      buscar_usuarios_excluidos(&quantidade_de_usuarios_excluidos);
...
IndiceUsuario *
buscar_usuarios_excluidos_area_de_extensao(int *quantidade_de_excluidos) {
  FILE *indice_exaustivo =
      fopen("./indices/indice_usuarios_exaustivo_area_de_extensao.bin", "rb");
  if (indice_exaustivo == NULL) {
    return NULL;
  }
  ...
  if (counter == 0) {
    printf("Nenhum usuário excluído no índice exaustivo de usuários da área de "
           "extensão.\n");
    free(usuarios_excluidos);
    return NULL;
  }
...
void juntar_usuarios_area_de_extensao() {
  FILE *usuarios_area_de_extensao =
      fopen("./areas_extensao/usuarios_area_extensao.bin", "rb");
  FILE *usuarios = fopen("./dados/dados_usuarios.bin", "a");

  int quantidade_de_excluidos = 0;
  IndiceUsuario *excluidos =
      buscar_usuarios_excluidos_area_de_extensao(&quantidade_de_excluidos);
  ...
  int quantidade_de_usuarios_area_de_extensao =
      ftell(usuarios_area_de_extensao) / sizeof(Usuario);
...
IndiceProduto *buscar_produtos_excluidos_area_de_extensao(
    int *quantidade_de_produtos_excluidos) {
  FILE *indice_exaustivo =
      fopen("./indices/indice_produtos_exaustivo_area_de_extensao.bin", "rb");
  if (indice_exaustivo == NULL) {
    return NULL;
  }
...
void juntar_produtos_area_de_extensao() {
  FILE *produtos_area_de_extensao =
      fopen("./areas_extensao/produtos_area_extensao.bin", "rb");
  FILE *produtos = fopen("./dados/dados_produtos.bin", "a");

  int quantidade_de_excluidos = 0;
  IndiceProduto *excluidos =
      buscar_produtos_excluidos_area_de_extensao(&quantidade_de_excluidos);
  ...
  int quantidade_de_produtos_area_de_extensao =
      ftell(produtos_area_de_extensao) / sizeof(Produto);
